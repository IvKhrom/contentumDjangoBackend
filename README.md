...
- Чаты с системой: пошаговый сбор параметров (см. файл "Промпт+параметры.txt")
- Ограничение: только один незавершённый чат у пользователя
- Временные чаты удаляются через 10 минут, если нет ответа
...
### 5. Очистка временных чатов

```bash
python manage.py cleanup_temp_chats --minutes 10
```
